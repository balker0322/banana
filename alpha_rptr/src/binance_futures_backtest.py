...
from alpha_rptr.src.util import logger, allowed_range, retry, delta, load_data, resample
from alpha_rptr.src.binance_futures_stub import BinanceFuturesStub
...
OHLC_DIRNAME = os.path.join(os.path.dirname(__file__), "ohlc/{}/{}")
# OHLC_FILENAME = os.path.join(os.path.dirname(__file__), "ohlc/{}/{}/data.csv")
OHLC_FILENAME = os.path.join(os.path.dirname(__file__), "../ohlc/{}")
ORDER_FILENAME = os.path.join(os.path.dirname(__file__), "../ohlc/order/{}")
...
class BinanceFuturesBackTest(BinanceFuturesStub):
    ...
    def __init__(self, account, pair, file=None):
        """
        constructor
        :account:
        :pair:
        ...
        self.data_file=file
        self.pair = pair
        order_log_file=self.__get_order_file()
        BinanceFuturesStub.__init__(self, account, pair=self.pair, threading=False, order_log_file=order_log_file)
    ...
    def show_result(self):
        ...
        DATA_FILENAME = self.__get_file()
        # self.symlink(DATA_FILENAME, 'html/data/data.csv', overwrite=True)
        self.symlink(DATA_FILENAME, 'alpha_rptr/html/data/data.csv', overwrite=True)
        # ORDERS_FILENAME = os.path.join(os.path.dirname(__file__), "../orders.csv")
        # ORDERS_FILENAME = os.path.join(os.path.dirname(__file__), ".../orders.csv")
        # ORDERS_FILENAME = os.path.join(os.path.dirname(__file__), "../ohlc/orders/{}").format(self.data_file)
        # self.symlink(ORDERS_FILENAME, 'html/data/orders.csv', overwrite=True)
        ORDERS_FILENAME=self.__get_order_file()
        self.symlink(ORDERS_FILENAME, 'alpha_rptr/html/data/orders.csv', overwrite=True)
    ...
    def __get_file(self):
        ...
        fname=OHLC_FILENAME.format('data.csv')
        if self.data_file:
            fname=OHLC_FILENAME.format(self.data_file)
        return fname
    
    def __get_order_file(self):
        # return self.data_file
        fname=ORDER_FILENAME.format('order.csv')
        if self.data_file:
            fname=ORDER_FILENAME.format(self.data_file)
        return fname
