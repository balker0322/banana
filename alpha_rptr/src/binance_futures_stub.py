...
from alpha_rptr.src.util import logger
...
class BinanceFuturesStub(BinanceFutures):
    ...
    def __init__(self, account, pair, threading=True, order_log_file="orders.csv"):
        """
        constructor
        :account:
        :pair:
        ...
        if order_log_file:
            file_name=order_log_file
        self.order_log = open(file_name, "w")
...
        """
        ...
        if (self.get_position_size() > 0 >= order_qty) or (self.get_position_size() < 0 < order_qty):
            ...
            if self.balance_ath < self.balance:
                    self.balance_ath = self.balance
            if self.balance_ath > self.balance:
                if not self.max_draw_down_session:
